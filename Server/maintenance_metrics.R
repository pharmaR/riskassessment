...
observe({
  ...
  if (input$tabs == "mm_tab_value") {
    if (!is.null(input$mm_comment)) {
      if(values$package_has_vignettes[2] == -1){ runjs( "setTimeout(function(){ notAvailable1('vignette');}, 500);" ) }
      if(values$package_has_website[2] == -1){ runjs( "setTimeout(function(){ notAvailable1('website');}, 500);" ) }
      if(values$package_has_news[2] == -1){ runjs( "setTimeout(function(){ notAvailable1('hasnews');}, 500);" ) }
      if(values$news_is_current[2] == -1){ runjs( "setTimeout(function(){ notAvailable1('newscurrent');}, 500);" ) }
      if(values$has_bug_reports[2] == -1){ runjs( "setTimeout(function(){ notAvailable1('bugtrack');}, 500);" ) }
      if(values$status_of_last_30_reported_bugs[2] == -1){ runjs( "setTimeout(function(){ notAvailable2('bugstatus');}, 500);" ) }
      if(values$exported_objects_with_documentation[2] == -1){ runjs( "setTimeout(function(){ notAvailable2('exporthelp');}, 500);" ) }
      if(values$source_code_is_public[2] == -1){ runjs( "setTimeout(function(){ notAvailable1('source_pub');}, 500);" ) }
      if(values$has_a_package_maintainer[2] == -1){ runjs( "setTimeout(function(){ notAvailable1('pack_maint');}, 500);" ) }
