        ...
        shiny::withProgress(
          ...
          max = n_pkgs + 2, # Tell the progress bar the total number of events.
          {
            ...
            if (input$report_format == "html") {
              ...
              file.copy(system.file('app/www', 'reportHtml.Rmd'), Report, overwrite = TRUE)
            } 
              ...
              file.copy(system.file('app/www', 'ReportDocx.Rmd'),
                        Report,
                        overwrite = TRUE)
              file.copy(system.file('app/www', 'read_html.lua'),
                        file.path(my_tempdir, "read_html.lua"), overwrite = TRUE)
              file.copy(system.file('app/www', 'images', 'user-tie.png'),
                        file.path(my_tempdir, "images", "user-tie.png"),
                        overwrite = TRUE)
              file.copy(system.file('app/www', 'images', 'user-shield.png'),
                        file.path(my_tempdir, "images", "user-shield.png"),
                        overwrite = TRUE)
              file.copy(system.file('app/www', 'images', 'calendar-alt.png'),
              ...
              if (!dir.exists(file.path(my_tempdir, "images")))
                dir.create(file.path(my_tempdir, "images"))
              file.copy(system.file('app/www', 'ReportPdf.Rmd'),
                        Report,
                        overwrite = TRUE)
              file.copy(system.file('app/www', 'read_html.lua'),
                        file.path(my_tempdir, "read_html.lua"), overwrite = TRUE)
