...
app_server <- function(input, output, session) {
  ...
  pre_auth <- getOption("pre_auth_user")
  ...
  if (user$pre_auth_user == TRUE) {
    ...
    user_id  <- getOption("login_creds")$user_id
    user_pwd <- getOption("login_creds")$user_pwd
    
    if (user_id %in% dat$user && dat$admin[which(dat$user == user_id)] == TRUE && 
  ...
  go_loginx <- go_login %>% debounce(1000)
