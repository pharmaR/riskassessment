...
In this configuration, an **admin** user has access to the entire application, a **lead** user has access to all privileges except administrative privileges (e.g. managaing user access), and a **reviewer** only has the privilege to add packages for consideration. An organization can customize these user roles to support their needs. For more information please reference the [Deployment](Deployment.html) vignette.
<font size="2">[return to top](User_Roles_and_Privileges.html)</font>