...
User roles package the privileges together to allow individuals users different, but not necessarily mutually exclusive, access to the application. The default configuration contains three user roles: **admin**, **lead**, **reviewer**. Here is a snippet from the `yml` file:
...
In this configuration, an **admin** user has access to the entire application, a **lead** user has access to all privileges except administrative privileges (e.g. managing user access), and a **reviewer** only has the privilege to add packages for consideration. An organization can customize these user roles to support their needs. For more information please reference the [Deployment](Deployment.html) vignette.
<font size="2">[return to top](User_Roles_and_Privileges.html)</font>