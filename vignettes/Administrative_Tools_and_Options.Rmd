...
</br>As on the bottom of the User table, There is also a `r fa(name = "square-check", fill = "red", height = "1em")` checkbox to select multiple users followed by</br> a "Force Selected Users to Change Password" button. This button is enabled when you select two or more users.
...
To apply these changes, your `ADMIN` will need to select 'APPLY DECISION RULES'. A pop-up will ask the user to review and confirm the selected automation rules:
