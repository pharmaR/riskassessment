...
pkg_GenInfo = db_fun(glue("SELECT * FROM package WHERE name ='{params$package}'"))
...
WHERE cum_id = '{params$package}'"))

pkgs_in_db <- db_fun(glue("SELECT cum_id FROM community_usage_metrics"))
...
metric_id <- db_fun(glue("SELECT id FROM metric WHERE name = 'covr_coverage';"))

# covr_coverage <- db_fun(glue(
# "SELECT value
