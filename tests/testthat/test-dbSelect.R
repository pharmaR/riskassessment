test_that("database has been defined and dbSelect function works", {
  ...
  # 1. file exists?
  testthat::expect_true(file.exists(file.path(devtools::wd(), db_name)))
  
  # 2. valid db? test this is a valid sqlite database
  testthat::expect_equal(readLines(file.path(devtools::wd(), db_name), n =1, warn = FALSE), "SQLite format 3")  
  ...
  tbls <- dbSelect(query, file.path(devtools::wd(), db_name))
  ...
  metric <- dbSelect(query, file.path(devtools::wd(), db_name))
