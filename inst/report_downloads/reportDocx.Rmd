...
```{r package_dependencies_cards}
  if('Package Dependencies' %in% params$report_includes) {
     cards <- params$dep_cards %>%
      ...
      dplyr::select(`Metric Name`, `Metric Description`, `Metric Value`, `Metric Score`) %>% 
      dplyr::mutate(`Metric Description` = stringr::str_replace_all(`Metric Description`, 'by Type', '<br/>by Type')) %>% 
      dplyr::mutate(`Metric Description` = stringr::str_replace_all(`Metric Description`, 'from Base', '<br/>from Base')) %>% 
     ...
     html_table_width <- function(kable_output, width){
      ...
      kableExtra::kbl(cards, format = 'html', booktabs = T, escape = F) %>% 
        html_table_width(c(175, 150, 200, 100))

      # kableExtra::column_spec(1, width_max = "1.75in") %>% 
      # kableExtra::column_spec(2, width_max = "1.5in")  %>% 
      # kableExtra::column_spec(3, width = "2.0in", width_min = "2.0in")  %>% 
      # kableExtra::column_spec(4, width_max = "1.0in") 
