...
if('Maintenance Metrics' %in% params$report_includes){
  ...
  # if (!requireNamespace("kableExtra", quietly = TRUE)) maint_out_tab else maint_out_tab %>%  kableExtra::column_spec(2, width = "3cm") #kableExtra::kable_styling(position = "left")  #%>%
    

  # tried to cut the url in half but that didn't work either
  # x <- "https://github.com/Biogen-Inc/tidyCDISC/"
  # stringr::str_wrap(x, 20)
  # cut_string <- function(x, len = NULL){
  #   if(is.null(len)) len <- floor(nchar(x)/2)
  #   paste(strsplit(sub("\\s+$", "", gsub('(.{20})', '\\1 ', x)), " ")[[1]], collapse = "\n")
  # }
  # cut_string(x = "https://github.com/Biogen-Inc/tidyCDISC/")
  
}
```

* Metrics whose score is `NA` will not impact the package {riskmetric} score
...
if (!cm_ind) {
  ...
  if('Community Usage Metrics' %in% params$report_includes){
    comm_out_tab <- params$com_metrics %>%
      dplyr::mutate(
        `Metric Name` = title,
        `Metric Description` = desc,
    ...
    if (!requireNamespace("kableExtra", quietly = TRUE)) comm_out_tab else comm_out_tab %>% kableExtra::kable_styling(position = "left") #%>% kableExtra::column_spec(2, width = "3cm")
  }
}
```
