...
params$maint_metrics %>%
   ...
   kableExtra::column_spec(2, width = "1.75in", latex_valign = "p") %>%
...
```{r package_dependencies_header}
  if(any(c('Package Dependencies', 'Dependency Comments') %in% params$report_includes)) {
    tagList(h2("Package Dependencies"))
  }
```
...
```{r package_dependencies_cards}
  if('Package Dependencies' %in% params$report_includes) {
     cards <- params$dep_cards %>%
      ...
      kableExtra::kable_styling("basic", latex_options = "hold_position", full_width = F, position = 'left') %>% 
      kableExtra::column_spec(3, width = "1.4in", latex_valign = "p")
...
`r if(any(c('Source Explorer Comments', 'Function Explorer Comments') %in% params$report_includes)) {"\\newpage "}`

```{r source_explorer}
if(any(c('Source Explorer Comments') %in% params$report_includes)) {
  tagList(h2('Source Explorer'),
          br(),
