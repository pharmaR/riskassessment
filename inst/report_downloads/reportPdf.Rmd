...
if('Maintenance Metrics' %in% params$report_includes){
  maint_out_tab <- params$maint_metrics %>%
    dplyr::mutate(
      `Metric Name` = title,
      `Metric Description` = desc,
      `Metric Value` = dplyr::case_when(
        name %in% c('has_website', 'has_source_control') ~ glue::glue("URL: {value}"),
  ...
  if (!requireNamespace("kableExtra", quietly = TRUE)) maint_out_tab else maint_out_tab #%>%  kableExtra::kable_styling(position = "left")  #%>% kableExtra::column_spec(3, width = "3cm")
  # 
